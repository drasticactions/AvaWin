// Derived from Avalonia.Themes.Fluent/ColorPaletteResourcesCollection.cs (MIT, see NOTICE.md).
﻿using System;
            ...
            (key, x) =>
            {
                ...
                if (key != ThemeVariant.Dark && key != ThemeVariant.Light)
                {
                    throw new InvalidOperationException(
                        $"{nameof(AvaWinTheme)}.{nameof(AvaWinTheme.Palettes)} only supports Light and Dark variants.");
                }
            },
            (_, x) =>
            {
